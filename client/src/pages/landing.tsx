...
const LuxuryStoryBlock = ({ image, icon, subtitle, title, text, reverse, variants }: LuxuryStoryBlockProps) => {
  return (
    <motion.div 
      variants={variants} 
      className={cn(
        // CLIENT FIX: Removed black/40, changed to dark brown/40
    ...
    >
      ...
      <div className="relative w-full md:w-1/2 h-40 md:h-full overflow-hidden shrink-0">
         ...
         {/* CLIENT FIX: Removed black gradients, changed to dark brown */}
...
const HeroScene = ({ progress, isActive }: { progress: MotionValue<number>; isActive: boolean }) => {
  ...
  return (
    <motion.div
      // CLIENT FIX: Changed bg-[#050606] to Brand Dark #2B1810
    ...
    >
      <video
        ...
        defaultMuted={true}
      />
      {/* CLIENT FIX: Gradient uses #2B1810 instead of black */}
...
const StoryScene = ({ isActive, onScrollPositionChange }: ScrollableSceneProps) => {
  ...
  if (isMobile) {
    return (
      <motion.div 
        // CLIENT FIX: Changed bg-[#0A0806] to Brand Dark #2B1810
...
const ProductScene = ({ data, isActive, direction }: { data: ProductData; isActive: boolean; direction: number }) => {
  ...
  return (
    <motion.div
      // CLIENT FIX: Changed bg-[#050606] to #2B1810 for Dark scenes
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: isActive ? 1 : 0, x: isActive ? 0 : -50 }} transition={{ delay: 0.5, duration: 0.8 }}>
            ...
            {isActive ? (
              <AnimatedText
                text={data.name}
                variant="fade-up"
                // CLIENT FIX: Ensure title text is readable on dark/light backgrounds
...
export default function LandingPage() {
  ...
  const sceneLabels = ['SAHARAN', 'STORY', 'EXPERIENCE', 'CLASSIC ED.', 'LIMITED ED.', 'SERVE']; 
  ...
  return (
    <>
      ...
      <main className="relative w-screen h-[100dvh] bg-[#2B1810] text-[#F5EFE6] overflow-hidden">
        ...
        {/* NEW RENDER LOGIC: Using AnimatePresence to handle the Gallery/Zoom Transitions */}
        <AnimatePresence initial={false} custom={direction} mode="popLayout">
          ...
          {currentSceneIndex === 3 && (
            <ZoomWrapper key="scene-3" isActive={true} index={3} currentIndex={currentSceneIndex} direction={direction}>
              <ProductScene data={{ id: 'classic', name: "DESERT ROSE CLASSIC EDITION", year: "2024", batch: "042", abv: "43%",
                ...
                botanicals: ["Wild Sage", "Saffron", "Juniper", "Rose Hip"], bottleImage: bottleClassic,
                  }} isActive={true} direction={direction} />
            </ZoomWrapper>
          )}

          {currentSceneIndex === 4 && (
            <ZoomWrapper key="scene-4" isActive={true} index={4} currentIndex={currentSceneIndex} direction={direction}>
              <ProductScene data={{ id: 'limited', name: "DESERT ROSE LIMITED EDITION", year: "2025", batch: "001", abv: "43%",
