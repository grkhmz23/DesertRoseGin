            ...
            <div className="absolute inset-y-0 -left-4 w-9 flex flex-col justify-between py-0">
              {Array.from({ length: totalScenes }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    {/* Simple line indicator - no rounded corners */}
                    <div className={cn(
                      "absolute right-[18px] h-[1px] transition-all duration-500",
                      isActive ? "w-6 bg-[#CD7E31] opacity-100" : "w-3 bg-[#F5EFE6]/40 group-hover:w-5 group-hover:bg-[#F5EFE6]/80"
                    )} />
                    {isActive && (
                      <motion.div 
                        layoutId="active-dot" 
                        className="absolute right-[18px] translate-x-1/2 w-1.5 h-1.5 bg-[#CD7E31] shadow-[0_0_10px_#CD7E31]" 
          ...
          <div className="hidden md:flex flex-col justify-between h-[320px] py-0 text-right">
            {labels.map((label, i) => (
              <button 
                ...
                onClick={() => onSelect(i)} 
                className={cn(
                  // UPDATED: Simple text, no boxes/borders/backgrounds
                  "font-hud text-[9px] uppercase tracking-[0.3em] transition-all duration-500 text-right h-4 flex items-center justify-end",
                  i === currentSceneIndex 
                    ? "text-[#CD7E31] opacity-100 translate-x-0" 
                    : "text-[#F5EFE6]/40 hover:text-[#F5EFE6]/80 -translate-x-1"
                )}
              >
                {label}
              </button>
